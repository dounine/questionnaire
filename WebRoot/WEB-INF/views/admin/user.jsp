...
<title>用户管理</title>
	...
	$(function() {
		...
		dataGrid = $('#dataGrid').datagrid({
			...
			columns : [ [ {
				...
				sortable : true,
				formatter : function(value, row, index) {
					var jsonObjs = $.parseJSON('${educaJson}');
					for(var i =0;i<jsonObjs.length;i++){
						var jsonobj = jsonObjs[i];
						if(jsonobj.id==value){
							return jsonobj.text;
				...
				sortable : true,
				formatter : function(value, row, index) {
					var jsonObjs = $.parseJSON('${divisionageJson}');
					for(var i =0;i<jsonObjs.length;i++){
						var jsonobj = jsonObjs[i];
						if(jsonobj.id==value){
							return jsonobj.text;
				...
				sortable : true,
				formatter : function(value, row, index) {
					var jsonObjs = $.parseJSON('${postJson}');
					for(var i =0;i<jsonObjs.length;i++){
						var jsonobj = jsonObjs[i];
						if(jsonobj.id==value){
							return jsonobj.text;
				...
				sortable : true,
				formatter : function(value, row, index) {
					var jsonObjs = $.parseJSON('${postlevelJson}');
					for(var i =0;i<jsonObjs.length;i++){
						var jsonobj = jsonObjs[i];
						if(jsonobj.id==value){
							return jsonobj.text;
	...
	function importFun(){
		if(orgId == null || orgId == ''){
			$.messager.alert("提示","请先选择左边的机构类型，再导入！","info",function(){
				return false;
			});
		}else{
			parent.$.modalDialog({
				...
				width : 500,
				height : 100,
				href : '${ctx}/user/importUser?orgId=' + orgId,
				buttons : [ {
					text : '导入',
					handler : function() {
						parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div data-options="region:'west',border:false,split:true" title="组织机构" style="width:200px;">
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/user/add')}">
			...
			<a onclick="importFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon_add'">excel导入</a>
		</c:if>
	</div>
</body>
</html>