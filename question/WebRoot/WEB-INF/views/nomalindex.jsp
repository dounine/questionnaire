...
var interval = 1000; 
	...
	function ShowCountDown(endDate)  { 
		...
		if(hour <=0 && minute <=0 && second<=0){ //超时，考试作废
			clea();
			$.messager.alert("糟糕","考试时间已到，答题作废！","warning",function(){
		...
		$.ajax({
           ...
           success: function(response){
               ...
               if (subjectCount > 0) { //有试题
               		$("#startID").hide();
               		$("#mainID").html('<br><h2>'+subDescript+'</h2><br>'+'<a id="btn"  class="easyui-linkbutton"  onclick="askBeginFun()" >开始答题</a>');
	           ...
	           success: function(objdata){
	        	  // $("#mainID").hide();
	        	   ...
	        	   mess =  ""+currIndex+". "+data.name+" "+
						   		...
						   		mess1 += "</br><br><a id='preBtn'  class='easyui-linkbutton'  onclick='askNextFun("+data.id+" , 0)' >上一题</a> "+
						   		"<a id='nextBtn'  class='easyui-linkbutton'  onclick='askNextFun("+data.id+" , 1)' >下一题</a>&nbsp;&nbsp;&nbsp;&nbsp;"+
						   		"&nbsp;&nbsp;&nbsp;&nbsp;<a id='subBtn'  class='easyui-linkbutton'  onclick='subFun()' >交卷</a>";
	...
	function submitSubject(){
    	...
    	//alert(subjectids+"========="+subjectids);
		 ...
		 $.ajax({
	           type: "post",
	           url: '${ctx}' + '/subjectmng/submitSub',
	           cache: false,
	           data: {paperid:paperid,subjectids:subjectids,answers:answers},
	           dataType: "text",
	           success: function(objdata){
	        	   if(objdata == 'succ'){
	...
	<div id="index_layout">
		...
		<div data-options="region:'center'" style="overflow: hidden;">
				<div title="问卷调查" data-options="border:false" style="overflow: hidden;padding:10px 0 20px 20px">
					...
					<div id="mainID" style="padding:10px 0 10px 10px;"> </div><br><br><br>
					<div id="askID1" style="position:absolute;bottom:80px;padding:0px 0 0px 0px;"> </div><br>
					<div id="timeID" style="position:absolute;bottom:50px;padding:10px 0 10px 10px;"> </div> </div><br>
